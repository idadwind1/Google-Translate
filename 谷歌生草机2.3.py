...
def start_translating():    #翻译的函数
    ...
    languages = list(googletrans.LANGUAGES.keys())  #所有语言
    for i in range(times):  #循环times次
        lang = languages[i%len(languages)]  #选择语言
        var_lang.set('当前语言：'+googletrans.LANGUAGES[lang])   #显示语言
        texts = translate(texts,dest=lang).text #翻译
        progress_bar['value'] = i+1 #设置当前进度
        window.update() #更新窗口
    texts = translate(texts,dest='zh-cn').text  #翻译成中文
    var_lang.set('当前语言：chinese (simplified)')   #设置var_lang
...
def paste():    #一键粘贴的函数
    ...
    lang = googletrans.LANGUAGES[googletrans.Translator(
        service_urls=['translate.google.cn']
    ).detect(str1).lang.lower()]
    var_lang.set('当前语言：'+lang)
button_paste = tk.Button(   #一键粘贴的按钮
    master=window,
    text='一键粘贴',
    font=('微软雅黑',20),
    command=paste
